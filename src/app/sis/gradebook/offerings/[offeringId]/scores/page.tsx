...
export default function OfferingScoresPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (orgLoading || !offeringId || !organizationId) return;
      try {
        setLoading(true);
        ...
        // Fetch graded items for this offering
        const itemsData = await listGradedItems({
          section_subject_offering_id: offeringId,
        });
        setItems(itemsData);

        // Fetch students in the section (homeroom section)
        const sectionStudents = await listSectionStudents(context.section_id);
  ...
  }, [orgLoading, offeringId, organizationId]);
  ...
  const handleBulkSave = async () => {
    if (!selectedItemId || !organizationId) {
      toast({
        title: "Error",
        description: "Please select a graded item first",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `Scores saved successfully for ${scoreRows.length} student(s)`,
      });
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Enter scores for {offeringContext.subject_name}
            </p>
          </div>
        </div>
      </div>
