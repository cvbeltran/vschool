...
import { getMasteryModel, updateMasteryModel } from "@/lib/mastery";
import { useToast } from "@/hooks/use-toast";

export default function EditMasteryModelPage() {
  ...
  useEffect(() => {
    const fetchModel = async () => {
      ...
      try {
        setLoading(true);
        const model = await getMasteryModel(modelId);
        if (model) {
          setFormData({
            name: model.name,
            description: model.description || "",
            is_active: model.is_active,
            threshold_not_started: model.threshold_not_started,
            threshold_emerging: model.threshold_emerging,
            threshold_developing: model.threshold_developing,
            threshold_proficient: model.threshold_proficient,
            threshold_mastered: model.threshold_mastered,
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      await updateMasteryModel(modelId, formData);
      toast({
        title: "Success",
        description: "Mastery model updated successfully",
      });
      router.push(`/sis/mastery/setup/models/${modelId}`);
    } catch (error: any) {
      toast({
        title: "Error",
        description: error.message || "Failed to update mastery model",
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-semibold">Edit Mastery Model</h1>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="sm" onClick={() => router.push(`/sis/mastery/setup/models/${modelId}`)}>
